...
io.on("connection", (socket) => {
  ...
  socket.on('joinRoom', async (name, room, type) => {
    ...
    if (!roomPlayers.has(room) && type == 2) {
      console.log(`${name}'s request to join room ${room} rejected (ROOM UNAVAILABLE)`);
      ...
      if (duelStatus == "Initialized") {
        ...
        if (start) {
          ...
          let ts = Date.now();
          io.to(players[0].id).emit('startDuel', players[0], players[1], ts, room, 1);
          io.to(players[1].id).emit('startDuel', players[0], players[1], ts, room, 2);
        }
      } else {
        //
      }
